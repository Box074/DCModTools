...
namespace Packer
{
	public class Bin2DMaxRects : Bin2D
	{
		...
		protected override bool InsertElement(uint _id, Size _elementSize, out Rectangle _area)
		{
            ...
            Bin2DMaxRects.Element element = new()
            {
                area = _area,
                id = _id
            };
			...
			{
				m_FreeAreas.Add(new Bin2DMaxRects.Element(rectangle));
			}
			...
			{
				m_FreeAreas.Add(new Bin2DMaxRects.Element(list[j]));
			}
			int k = 0;
		...
		{
			foreach (Bin2DMaxRects.Element element in m_UsedAreas)
			{
		...
		{
			m_FreeAreas = new List<Bin2DMaxRects.Element>();
			m_UsedAreas = new List<Bin2DMaxRects.Element>();
			m_FreeAreas.Add(new Bin2DMaxRects.Element(new Rectangle(0, 0, base.size.Width, base.size.Height)));
		}
