...
#pragma warning restore CS8622 // 参数类型中引用类型的为 Null 性与目标委托不匹配(可能是由于为 Null 性特性)。
        ...
        public async Task<string> GetOriginalResPath(Window? parent = null)
        {
            ...
            if (string.IsNullOrEmpty(originalExpanedResPath))
            {
                ...
                {
                    Title = "Open Original Expanded ModHelperSkin Folder"
                };
                var str = await openFolderDialog.ShowAsync(parent ?? MainWindow.mainWindow);
                if (str == null)
                {
                    var result = await MessageBox.Avalonia.MessageBoxManager.GetMessageBoxStandardWindow("Error", "Original ModHelperSkin is required, do you want to quit the program?", MessageBox.Avalonia.Enums.ButtonEnum.YesNo,
